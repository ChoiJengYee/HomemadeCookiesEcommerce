...
public class WishlistRepository
{
    ...
    {
        const string sql = "SELECT wi.wishlist_item_id, wi.wishlist_id, wi.cookie_id, wi.added_at, c.name FROM wishlist_items wi INNER JOIN wishlists w ON w.wishlist_id = wi.wishlist_id INNER JOIN cookies c ON c.cookie_id = wi.cookie_id WHERE w.customer_id = @customerId ORDER BY wi.added_at DESC";
        await using var connection = DatabaseConnection.Instance.CreateConnection();
        await connection.OpenAsync(cancellationToken);
        await using var command = new NpgsqlCommand(sql, connection);
        command.Parameters.AddWithValue("customerId", customerId);
